...
function displayNextDayWeather() {
  let nextDays = Array.from(
    document.getElementsByClassName("weather-next-day-card")
  );
  for (let i = 0; i < nextDays.length; i++) {
    if (days.length == date.getDay() + 1) {
      document.getElementsByClassName("next-day")[i].innerHTML = days[i];
    } else {
      document.getElementsByClassName("next-day")[i].innerHTML =
        days[date.getDay() + i + 1];
    }
    if (months.length == date.getMonth() + 1) {
      document.getElementsByClassName("next-date")[i].innerHTML =
        +date.getDate() + i + 1 + "  " + months[i];
    } else {
      document.getElementsByClassName("next-date")[i].innerHTML =
        +date.getDate() + i + 1 + "  " + months[date.getMonth() + i + 1];
    }
    document.getElementsByClassName("next-days-display")[i].src =
      "https://" + weather.forecast.forecastday[i + 1].day.condition.icon;
    document.getElementsByClassName("next-day-max-temp")[i].innerHTML =
      weather.forecast.forecastday[i + 1].day.maxtemp_c + "<sup>o</sup>C";
    document.getElementsByClassName("next-day-min-temp")[i].innerHTML =
      weather.forecast.forecastday[i + 1].day.mintemp_c + "<sup>o</sup>C";
    document.getElementsByClassName("condition")[i].innerHTML =
      weather.forecast.forecastday[i + 1].day.condition.text;
  }
}

//Email Input Validator
function emailSubscribeValidator() {
  emailSubscribe.addEventListener("keyup", () => {
    if (Emailregex.test(emailSubscribe.value) == false) {
...
//Subscribe Button Validator
function subscribeBtnValidator() {
  subscribeBtn.removeAttribute("data-bs-toggle");
  subscribeBtn.addEventListener("click", () => {
    if (Boolean(emailSubscribe.value) == false) {
      document.getElementsByClassName("invalid")[0].innerHTML =
        "Please enter e-mail to subscribe";
      document.getElementsByClassName("invalid")[0].classList.remove("d-none");
    }
  });
}

//subscribe e-mail validator Calling
(function validator() {
  emailSubscribeValidator();
  subscribeBtnValidator();
})();
